...
import { parseLanguagesToml, type LanguageMetadata, type LanguagesToml } from "./languages-toml.js";
...
function languageInfo(id: string, entry: LanguageMetadata) {
  ...
  return {
    ...
    name,
    aliases,
    extensions: globs.filter((glob) => glob.startsWith("*.")),
    globs,
    emacsModes: entry.emacs ?? [],
    shebangs: entry.shebang ?? [],
  };
}
...
`;
  ...
  const plaintextAliases = config.plaintext.aliases ?? [];
...
`;
  ...
  for (const [id, entry] of Object.entries(config.parsers)) {
    languageEntries.push(languageInfo(id, entry));
  }

  languageEntries.push(languageInfo("plaintext", config.plaintext));
...
`;
  ...
  for (const [bundleName, bundleEntry] of Object.entries(config.bundles ?? {})) {
    bundleMembers[bundleName] =
      bundleEntry.parsers === "all" ? [...allParserIds] : [...bundleEntry.parsers];
  }
