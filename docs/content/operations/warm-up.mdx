...
document names on demand. So it should not be able to delay a boot or fail one,
and the examples below are written to keep both true.
...
| --- | --- | --- |
| Elixir | `Lumis.Languages.async_load/1` from `start/2` | parser metadata, verified WASM, and compiled Wasmtime modules |
| Server-side JavaScript | `cacheLanguages()`, not awaited, alongside startup | parser metadata and verified WASM; compiled Wasmtime modules when the native addon is available |
...
Start serving, then warm in the background:
...
Await it instead when something downstream should not start without a prepared
cache, such as a build step or a prestart task. In a serving process, awaiting
it before `startServer()` means an unreachable CDN keeps the process from ever
accepting a request.
...
`cacheLanguages()` fills the store rather than loading languages into the
runtime, so the first document naming one still loads it — from disk, with the
compile already done. For a highlighter you reuse, `createHighlighter({
languages })` warms and keeps them in the instance.
