...
The installed binary is `lumis`. npm resolves it from a platform package, so the install runs no scripts and downloads nothing beyond the registry. Linux musl hosts have no package yet and need `cargo install lumis-cli`, which Rust users can prefer anywhere.
