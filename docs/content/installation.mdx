...
Use `@lumis-sh/lumis` for the default per-language WASM runtime and `@lumis-sh/themes` for bundled themes.

For the optional native Node.js runtime:

```bash
npm install @lumis-sh/lumis-native
```

Lumis detects it automatically. The native addon contains all supported language parsers, so it has a much larger install size than the default WASM setup.
...
- On supported Node.js platforms, `@lumis-sh/lumis-native` replaces per-language parser WASM with an opt-in native addon containing all parsers.
