...
- `Lumis.Languages.load/1` and `mix lumis.languages.cache` in Elixir, `lumis languages cache` for the CLI, and `npx lumis-languages-cache` for Node all move the download ahead of the first request. They also move native Wasmtime compilation when that runtime is available.
