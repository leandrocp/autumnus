...
- Call `Lumis.Languages.cache/2` or JavaScript's `cacheLanguages()` during application startup to move the download and native Wasmtime compilation ahead of the first request. Use `lumis languages cache` when a standalone CLI step is preferable.
