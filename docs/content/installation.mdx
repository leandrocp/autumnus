...
- JavaScript loads parser WASM files at runtime, persists verified bytes in Node or browser caches, and supports build-time prefetch. See [WASM and CDN](/advanced/wasm-and-cdn).
- Elixir loads missing root and injected parser WASM on demand; OTP releases can embed selected parsers with `mix lumis.parsers.fetch`.
