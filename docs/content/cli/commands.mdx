...
| `--data-dir <path>` | | Override the data directory (also `LUMIS_DATA_DIR` env var) |
...
| `--decorator <decorator>` | `-d` | Built-in terminal decorator. Supports `rainbow-brackets`. |
...
lumis highlight main.rs -d rainbow-brackets
```
...
Prints all supported languages with their file decorators and patterns.
