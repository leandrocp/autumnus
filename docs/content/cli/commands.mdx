...
Download exact, integrity-checked parser WASM files ahead of time. Highlighting
downloads what it needs anyway, so this is to keep that cost off the first run,
or to keep the download out of the first run.
