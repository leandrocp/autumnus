...
- loads each block's detected language before rendering it, so a document decides which parsers are fetched; pass a fixed language set or a preset bundle if that matters to you
