...
- every language pins an exact parser version, size, and SHA-256 digest
- Node caches verified parser WASM in the platform user cache directory; `LUMIS_WASM_CACHE_DIR` overrides it
- browsers persist verified parser bytes in CacheStorage with an IndexedDB fallback
...
Both paths use the exact manifest version and verify size plus SHA-256 before
writing. `LUMIS_WASM_OFFLINE=1` turns a missing parser into an error instead of
a network request.
