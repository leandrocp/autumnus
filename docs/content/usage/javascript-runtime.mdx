...
- on Node, `@lumis-sh/lumis` runs those parsers under Wasmtime in a native addon and walks and formats in Rust, the same code the Lumis CLI and Elixir bindings run, so all three produce identical output
...
That one-pass loading is done by the Rust store. A custom
[`configureWasmResolver`](/advanced/wasm-and-cdn#override-the-resolver) applies
to the languages you load, not to one discovered mid-document: `highlight()` is
synchronous, so there is nowhere to await the URL a resolver returns. With a
custom resolver, load injected languages up front, prefetch them with
`cacheLanguages()`, or point `LUMIS_WASM_PATH` at a directory containing them.

In a browser this does not happen: loading is asynchronous there and cannot run
inside a synchronous walk, so load every participating language yourself, or use
a bundle such as `@lumis-sh/wasm-bundle-web`:
...
## Prepare parsers for deployment

For a host with no network access, populate a project or image-local cache
...
The equivalent API is available from `@lumis-sh/lumis/cache`:
...
Both paths cache the current language package and verify its exact parser size
plus SHA-256 before writing. A cached parser is served without a network
request, so caching every language a deployment needs makes it self-contained.
