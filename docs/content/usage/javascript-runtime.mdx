...
- browser usage usually relies on remote parser assets unless you override the resolver, use `withWasm()` with a single npm parser package, or use `withWasmBundle()` with a bundle package such as `@lumis-sh/wasm-bundle-web`
