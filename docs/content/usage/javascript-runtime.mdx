...
| Bundle | Purpose |
| --- | --- |
| `@lumis-sh/lumis/bundles/essential` | docs, config, shell, diff, and other low-level essentials |
...
- Node can also auto-load installed parser packages such as `@lumis-sh/wasm-elixir` or preset packages such as `@lumis-sh/wasm-bundle-essential`
- browser usage usually relies on remote parser assets unless you override the resolver, use `withWasm()` with a single npm parser package, or use `withWasmBundle()` with a preset bundle package
