...
To keep the download and compile off a first request, populate a project or image-local cache
...
Both accept `bundle-*` names as well, covering the same languages as the
...
manifest and verify its parser size plus SHA-256 before writing. A cached parser
is served without a network request, so caching every language a deployment
needs makes it self-contained. Pass `--force` to the command, or `force: true`
to the API, to resolve the range again and adopt a newer compatible package.
When the native Node addon is available, both paths also validate each parser
and its queries and persist the compiled Wasmtime module in the same directory.
The portable fallback still caches the verified parser bytes, but
