...
Awaiting it before `startServer()` puts the CDN on the boot path, so an
unreachable one keeps the process from ever accepting a request. Await it in a
build or prestart step instead, where failing is the point.
...
Use `lumis languages cache`, or an awaited `cacheLanguages()`, when cache
preparation belongs in a separate build or operations step.
