...
- the package uses conditional exports so browsers and Node get the right runtime automatically
...
- browser usage usually relies on remote parser assets unless you override the resolver, use `withWasm()` with a single npm parser package such as `@lumis-sh/wasm-json`, or use `withWasmBundle()` with a bundle package such as `@lumis-sh/wasm-bundle-web`
- Bun and Deno are also supported
- Edge runtimes such as Cloudflare Workers or Vercel Edge follow the browser runtime path, so the best DX is to use npm WASM packages with `withWasm()` or `withWasmBundle()` instead of manually fetching parser assets
