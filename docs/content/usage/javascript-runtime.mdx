...
## Prepare parsers during application startup

Call the cache API in your startup lifecycle before the application begins
accepting requests:
...
Use `lumis languages cache` when cache preparation belongs in a separate build
or operations step instead of the application lifecycle.
