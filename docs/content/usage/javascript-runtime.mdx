...
- Node caches language-package metadata and verified parser WASM in the platform user cache directory; `LUMIS_WASM_CACHE_DIR` overrides it
...
Do this for every language you expect to render. Loading one on demand makes
the request that needed it wait for a download and a grammar compile — see
[Load what you intend to highlight](/advanced/wasm-and-cdn#load-what-you-intend-to-highlight).
For Node deployments, populate a project or image-local cache before the
application starts:
