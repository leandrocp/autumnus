...
Lumis.highlight!(code, formatter: {:html_inline, lang: "lib/my_app.ex"})
Lumis.highlight!(code)
```

Omitting `lang` from the formatter enables auto-detection.
...
```elixir
code = Lumis.highlight!(source, formatter: {:html_inline, lang: "elixir"})
```
...
Lumis.validate_options!(formatter: {:html_inline, lang: "elixir", theme: "dracula"})
