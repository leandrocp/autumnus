...
Lumis loads missing languages automatically. Parser WASM is packaged with
matching queries and integrity metadata, then cached in the platform user cache
directory. It first checks release-local `priv/wasm`, so production releases do
not need a runtime download.
...
Load every language you expect to render during application startup. On-demand
loading makes the first request that needs a language wait for its download and
compile — see [Cache and load in advance](/advanced/wasm-and-cdn#cache-and-load-in-advance).
