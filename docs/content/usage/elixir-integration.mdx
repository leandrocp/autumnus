...
### Preparing parsers during application startup

A cold parser costs a download and then a Wasmtime compile, and the compile is
the larger half. Cache the languages your application needs before starting its
supervision tree so a request never pays either:
...
same persistent directory reuses them. Pass `force: true` to resolve the
compatible package range again and replace files that already verify.

Use `Lumis.Languages.load/1` instead when you also want to retain the languages
in the current VM. Use the standalone `lumis languages cache` CLI when cache
preparation belongs in an image build or operations step.
...
| Removed | Use instead |
| --- | --- |
| `config :lumis, :bundled_languages` | call `Lumis.Languages.cache/2` during application startup |
...
`Lumis.Languages.cache/2` takes language names or a `:bundle_*` name rather than
