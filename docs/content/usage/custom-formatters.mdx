...
Decorators add things that are not syntax tokens. Today that means inline span decorations. A formatter that wants those should render a `LineView`, not just the raw token stream.
...
At render time, handle the parts your formatter supports:

- `Line.line_highlights` for whole-line state owned by formatter configuration.
- `Line.signs`, `Line.gutter_text`, and `Line.virtual_text` for custom renderer data that you add through `LineViewOptions`.
...
use lumis::{
    ...
    formatters::html,
    highlight::{
        DecoratorContext, DecorationOutput, HighlightDecoration, LineView, LineViewBuilder,
...
If your formatter uses `highlight_iter()` directly, it can still render syntax highlighting. It just will not see `LineView` span decorations or line-view options.
