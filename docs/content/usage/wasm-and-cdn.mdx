...
For multiple languages, install a preset package such as `@lumis-sh/wasm-bundle-essential` and pair it with the matching Lumis bundle.
...
import githubLight from '@lumis-sh/themes/github_light'
import '@lumis-sh/wasm-bundle-essential'

const hl = await createHighlighter({
  languages: [bundledLanguages],
})

await hl.loadLanguage(bundledLanguages.toml)
...
Use this when you want one package in `package.json` to cover a whole preset. Lumis still imports only `@lumis-sh/lumis/bundles/essential`; Node resolves the installed parser packages automatically.

See `examples/wasm-bundle-essential-node`.
...
await hl.loadLanguage(languages.toml)
...
See `examples/wasm-bundle-essential-browser`.
...
- platform and package overview: [Platforms](/reference/platforms)
