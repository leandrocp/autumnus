...
parser and its queries in a disposable Tree-sitter store; the Node cache
command/API does the same wherever its native addon is available. Wasmtime's
compiled forms then travel with the cache without retaining the full catalog in
memory, so the first native request pays neither the download nor the compile.
...
- run the cache command with `--force` to resolve the range again and adopt a
  newer compatible language release without upgrading Lumis
