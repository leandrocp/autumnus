...
### Preparing during application startup

A cold parser costs a download and then a Wasmtime compile. The compile is the
larger half, so call the host API before accepting requests rather than letting
...
await cacheLanguages(["rust", "javascript", "elixir"]);
await startServer();
```
...
Both APIs are idempotent and reuse verified parser bytes on later starts. Native
hosts also reuse their compiled modules. `Lumis.Languages.load/1` and
JavaScript's highlighter setup are still available when an application also
wants to retain languages in memory.
