...
- each language pins the exact parser package version, byte length, and SHA-256 digest

## Default JavaScript behavior

Lumis resolves parser WASM from jsDelivr using exact package versions.
...
Lumis rejects bytes whose size or SHA-256 digest differs from the language
manifest. Resolution follows a persistent hierarchy:

```text
installed or deployment-local parser
  -> verified platform cache (CacheStorage or IndexedDB in browsers)
  -> exact-version CDN fallback
```

Set `LUMIS_WASM_CACHE_DIR` to override the Node cache. Set
`LUMIS_WASM_OFFLINE=1` to prohibit the final network fallback.
