...
A cold parser costs a download and then a Wasmtime compile. The compile is the
larger half — seven parsers already on disk take about 8 s to compile, and 1.3 s
once their compiled forms are cached — so load ahead of time rather than letting
a first request pay for either:
...
A language import is a package handle: `packageName` selects its `language.json`,
...
planned feature, not a supported one. It previously existed in JavaScript only
...
$LUMIS_DATA_DIR/
  parsers/
    elixir.language.json                              # package metadata
...
### Versions are pinned, so nothing is revalidated

Each Lumis release records the exact package version it expects, and asks the
CDN for that version rather than `@latest`. Two consequences worth relying on:

- **A request never waits on the network for something already on disk.** A
  package in the directory is the pinned one, so there is nothing to check.
- **Two machines render a document identically.** Under `@latest`, which parser
  you got depended on when your cache was last refreshed.

Parser updates therefore arrive when you upgrade Lumis, not on their own. If a
pinned version has been unpublished, Lumis warns and falls back to `@latest` so
the language still works, but that machine is no longer guaranteed to match the
others.

### Warming it at build time

Because the runtime only reads this one directory, filling it during a build is
all that pre-warming requires:

```dockerfile
ENV LUMIS_DATA_DIR=/app/lumis
RUN mix lumis.languages.cache elixir heex markdown json
...
const hl = await createHighlighter({
  ...
  languagePackageResolver: (packageName) =>
    `/wasm/${encodeURIComponent(packageName)}/language.json`,
