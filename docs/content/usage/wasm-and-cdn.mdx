...
## Cache and load in advance

Loading a language on demand costs a metadata request, a parser download, and a
grammar compile, all while the request that triggered it waits. Do it at build
or boot time instead, for every language you expect to render:
...
Injected languages count too: highlighting Markdown with fenced Rust needs the
Rust parser, and an HTML page needs CSS and JavaScript.
