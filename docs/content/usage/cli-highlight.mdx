...
- omit it when file decorator, filename, or source content should drive detection
...
Use `--decorator` (or `-d`) for built-in rendering decorators. This first slice supports rainbow brackets in terminal output.

Decorators use the same line-view output model as the Rust API. For now, CLI decorator output is rendered only by the terminal formatter.

```bash
lumis highlight main.rs --decorator rainbow-brackets
```

See [LineView Decorations](/usage/line-view-decorations) for the Rust API behind this model.
