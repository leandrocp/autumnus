...
Token classes use the `lumis-` prefix. Dots in Tree-sitter highlight scopes become dashes, so `comment` becomes `.lumis-comment`, and `tag.attribute` becomes `.lumis-tag-attribute`.

The block and language classes stay bare: `pre.lumis` and `language-*`.
