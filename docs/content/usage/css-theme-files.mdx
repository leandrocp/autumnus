...
Token classes use the `l-` prefix. Dots in Tree-sitter highlight scopes become dashes, so `comment` becomes `.l-comment`, and `tag.attribute` becomes `.l-tag-attribute`.
