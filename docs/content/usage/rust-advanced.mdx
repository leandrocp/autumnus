...
Use `LineViewBuilder` when you need a structured view before rendering. It runs highlighting internally and returns a `LineView` with spans, ranges, gutter text, virtual text, and decorator output.

```rust
use lumis::{highlight::LineViewBuilder, languages::Language};
...
assert_eq!(view.lines[1].gutter_text[0].text, "2");
```

This is the Rust entry point for formatter-neutral decorators. See [LineView Decorations](/usage/line-view-decorations).
