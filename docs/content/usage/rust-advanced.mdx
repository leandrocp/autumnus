...
### [`LineViewBuilder`](/usage/line-view-decorations)

Use `LineViewBuilder` when you need the structure before rendering. It runs highlighting and returns a `LineView` with spans, ranges, line data, virtual text, and decorator output.

```rust
use lumis::{highlight::{LineViewBuilder, RainbowBracketsOptions}, languages::Language};
...
This is the Rust entry point for decorators. Rainbow brackets use it today. Custom Rust decorators can emit the same `DecorationOutput`, and more built-ins can use this model later. See [LineView Decorations](/usage/line-view-decorations).
...
This is also useful for advanced custom formatters that only need token callbacks. Use `LineViewBuilder` when the formatter needs decorator output.
