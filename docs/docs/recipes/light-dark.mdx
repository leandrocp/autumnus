...
That's it. No JavaScript, no media queries. The `light-dark()` function resolves the correct variable automatically.
...
The `default_theme: "light"` makes the light theme's colors render as inline styles (visible without any CSS). The media query swaps to the dark theme's CSS variables when the OS is in dark mode.
...
This pattern works with any framework. In Phoenix LiveView, you'd wire the button to a `phx-click` event that toggles a class via a JS command.
