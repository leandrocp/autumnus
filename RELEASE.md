...
The `npm-cli` workflow builds the `lumis` binary for six targets, publishes the `@lumis-sh/cli-*` platform packages, and then publishes `@lumis-sh/cli` itself. All of them share the `npm-cli` version and tag, and `mise run release-prepare npm-cli <version>` bumps them together, for the same reason the addon packages move in lockstep: `@lumis-sh/cli` pins them with `workspace:*`, which pnpm rewrites at pack time. `mise run lint` runs `mise run check-cli-versions`, which fails if any of them drift.
...
Linux musl has no CLI platform package. The addon builds musl through a `rust:alpine` container because Node has to `dlopen` it; the CLI is a plain binary and would not need that, but the target is untested here. Adding it means a seventh and eighth package, the matching entries in `bin/target.js`, `check-cli-versions.mjs` and `scripts/test-shim.js`, and a build that links wasmtime against musl.
